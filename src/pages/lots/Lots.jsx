...
function Lots() {
  ...
  const filtered = useMemo(
    () =>
      lots.filter((lot) =>
        lot.company.name.toLowerCase().includes(searchValue.toLowerCase())
      ),
    [searchValue, lots]
  );
      ...
      {loading ? (
        ...
        <div className="py-4">
          {filtered.length ? (
                ...
                >
                  <div className="grid md:grid-cols-4 grid-cols-2 gap-4 w-full ">
                    <div>
                      ...
                      <p className="title">{lot.company.name}</p>
                    </div>
                    <div>
                      <p className="heading">Spaces</p>
                      <p className="title">{lot.spaces}</p>
                    </div>
                    <div>
                      <p className="heading">Width</p>
                      <p className="title">{lot.width}</p>
                    </div>
                    <div>
                      <p className="heading">Height</p>
                      <p className="title">{lot.height}</p>
                    </div>
                  </div>
                  <div>
                    <button
            ...
            <div className="text-center mt-10 text-white">No lots found!</div>
          )}
        </div>
      )}
    </div>
